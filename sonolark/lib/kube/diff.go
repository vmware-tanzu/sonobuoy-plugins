...
func kubeDiffFn(t *starlark.Thread, b *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	...
	obj1, gvk1, err := decode([]byte(maybeObj.(starlark.String)))
	if err != nil {
		return nil, fmt.Errorf("item %d is not a YAML string (got: %s): %v", 0, maybeObj.Type(), err)
	}

	maybeObj = args.Index(1)
	obj2, _, err := decode([]byte(maybeObj.(starlark.String)))
	if err != nil {
